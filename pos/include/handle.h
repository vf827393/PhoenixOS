...
namespace pos_protobuf { class Bin_POSHanlde; }
...
class POSHandle {
 ...
 protected:
     ...
     *  \return POS_SUCCESS for successfully checkpointed
     */
    virtual pos_retval_t __commit(
        uint64_t version_id, uint64_t stream_id=0, bool from_cow=false, bool is_sync=false, std::string ckpt_dir=""
    ){ 
        return POS_FAILED_NOT_IMPLEMENTED;
    }
    ...
    /*!
     *  \brief  serialize the common fields of POSHandle using protobuf
     *  \param  base_binary poiner to the basic fields to be operated
     *  \param  ckpt_slot   checkpoint slot of the state (if any)
     *  \return POS_SUCCESS for successfully serialized
     */
    pos_retval_t __serialize_protobuf_handle_base(pos_protobuf::Bin_POSHanlde* base_binary, POSCheckpointSlot* ckpt_slot=nullptr);
    /* ==================== checkpoint add/commit/persist ==================== */
 ...
 protected:
     ...
     */
    virtual pos_retval_t __sync_stream(uint64_t stream_id=0){
        return POS_FAILED_NOT_IMPLEMENTED;
    }
    /* ===================== platform-specific functions ===================== */

...
class POSMetrics_ReducerList {
 public:
    ...
    inline double get_reduce_avg(K index){
        auto it = this->_avg_map.find(index);
        if(likely(it != this->_avg_map.end())){
    ...
    inline std::string str(std::map<K,std::string>& reducer_names){
        ...
        for(
            ...
            POS_ASSERT(avg_map_iter != this->_avg_map.end());
            ...
            print_string += std::string("  avg: ") 
                            + std::to_string((double)(avg_map_iter->second)/(double)(avg_counter_map_iter->second))
                            + std::string("\n");
        }
 ...
 private:
    ...
    std::unordered_map<K, uint64_t> _avg_map;
